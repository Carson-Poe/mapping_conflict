...
We will explore the data set from the [Upsalla Data Conflict Program](https://ucdp.uu.se/#/). 

To start, download and clean the data. Then some light analysis and plotting. 
...
no_unk_df <- c_df %>% 
    ...
    select(id, type_of_violence, conflict_new_id, date, side_a, 
    side_b, number_of_sources, latitude, longitude, country, region, 
    event_clarity, deaths_a, deaths_b, civ_cat) %>% 
    mutate_if(is.character, factor) %>% 
    mutate(type_of_violence = factor(type_of_violence), 
           conflict_new_id = factor(conflict_new_id))
...
c_rec <- recipe(civ_cat ~ ., data = c_train) %>% 
    ...
    # Reduce categories
    step_other(side_a, side_b, country, conflict_new_id, threshold = .02) %>% 
...
knitr::kable(juice(c_prep), align = 'c', caption = 'Prepped for Modeling')
...
The deaths are the most important variables for predicting civilian causualties. 
Specifically the deaths of 'side_b'. It's hard to say exactly why. Is it becuase
with higher deaths from side B there's more likely to be civilian causualties? Or
is it becuase civilian deaths can be lumbed in with side_b, leading to 'no' civilian deaths but high side_b deaths? This is, of course, complete and uniformed conjecture. 
...
knitr::kable(final_res %>% 
    collect_metrics(), align ='c', caption = 'Metrics')
